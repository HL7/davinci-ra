...
Draft $risk-adjustment operation. The intent is to rename the existing $report operation to $risk-adjustment operation and to add a new required input parameter `type`.
...
The $risk-adjustment operation <b>SHALL</b> be able to query:
...
The $risk-adjustment operation <b>SHALL</b> be able to resolve:
...
The $risk-adjustment operation <b>SHALL</b> include the following output (OUT) parameters:
...
- If the patient is valid and no MeasureReport resources match the set of input (IN) parameters being processed, then the `Bundle.entry` SHALL include a [US Core Patient](http://hl7.org/fhir/us/core/STU3.1.1/StructureDefinition-us-core-patient.html) resource matching the subject (Patient/[reference]) (note: an OperationOutcome will NOT be returned because $risk-adjustment is to return existing MeasureReports and none exist in this scenario); <b>OR</b>
